...
import { createQuoteBoxSchema } from '~/server/utils/validations-schemas/dashboard/boxes/types/quote';
...
type QuoteFormStore = FormStoreApi<
	{ content: string; cite: string },
	typeof createQuoteBoxSchema
>;
...
const QuoteBoxEditOverlay = (props: Props) => {
	...
	const quoteFormStore: QuoteFormStore = useCreateFormStore({
		...
		validationSchema: createQuoteBoxSchema,
		...
		<QuoteBoxView
			...
			childrenAfter={
				<QuoteBoxEditOverlay
					boxDeepLevel={props.boxDeepLevel}
					box={props.box}
					path={props.path} // {[...props.path, 'quoteBox']}
					pageStore={props.pageStore}
					style={props.style}
				/>
			}
